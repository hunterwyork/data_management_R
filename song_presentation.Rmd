...
:::: column
  ...
  > - Second-year PhD student in Sociology at Princeton
  > - Office of Population Research Affiliate
  > - Formerly: MPH at UW and researcher at Institute for Health Metrics and Evaluation
  > - Formerly: AB at Harvard in  Human Evolutionary Biology and Music
...
```{r slide8.11, time_it = T, warning=F, error=F, message=F}
modr(vars_vec[[1]], c.dat = new_data, c.outcome = "inc_2019")
```


### Parallel - 2 Iterations


```{r slide8.4, time_it = T, warning=F, error=F, message=F}
parallel::mclapply(vars_vec[1:2], modr, c.outcome = "inc_2019",
                   c.dat = new_data, mc.cores = 4) %>% 
  rbindlist() -> all_betas
```
...
```{r slide8.5, time_it = T, warning=F, error=F}
lapply(vars_vec[1:2], modr, c.outcome = "inc_2019",
                   c.dat = new_data) %>% 
  rbindlist() -> all_betas
```
